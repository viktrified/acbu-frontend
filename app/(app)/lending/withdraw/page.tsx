...
export default function LendingWithdrawPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await lendingApi.lendingWithdraw({ lender: lender.trim(), amount }, opts);
      ...
      <PageContainer>
        <Card className="border-border p-4 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading || !lender.trim() || !amount}>Withdraw</Button>
          </form>
        </Card>
      </PageContainer>
    </>
